...
export async function readVisits() {
  try {
    ...
    console.log(
      `[Visits] Read ${visits.length} visits from Redis, ${filteredVisits.length} after filtering localhost`
    );
...
export async function recordVisit(request, path = "/") {
  try {
    ...
    // Skip localhost IPs (::1 for IPv6 localhost)
    if (ip === "::1" || ip === "127.0.0.1" || ip === "localhost") {
      console.log(`[Visits] ⏭️ Skipping localhost visit from ${ip}`);
      return;
    }
...
export async function getStats() {
  try {
    ...
    // Return all visits (pagination handled in frontend)
    const recentVisits = visits.slice().reverse();
